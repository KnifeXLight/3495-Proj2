...
def run_analytics():
    ...
    try:
        # 1. Read Raw Data from MySQL (Updated to grades table)
        mysql_conn = get_mysql_connection()
        cursor = mysql_conn.cursor(dictionary=True)
        cursor.execute("SELECT score FROM grades")
        rows = cursor.fetchall()
        cursor.close()
        mysql_conn.close()

        if not rows:
            print("No grades found in MySQL. Skipping.", flush=True)
            return
        ...
        stats = {
            "class_highest": max(values),
            "class_lowest": min(values),
            "class_average": round(sum(values) / len(values), 2),
            "total_grades_entered": len(values),
            "timestamp": time.time(),
        }
